...
static void stencil_basic(double *newa, const double *olda, int Nx, int Ny)
{
...
int main(int argc, char **argv)
{
    ...
    for (int t = 0; t < steps; t++) {
        ...
        stencil_basic(newa, olda, Nx, Ny);
        double *tmp = olda; olda = newa; newa = tmp;
    }
    double t1 = wall_seconds();

    printf("stencil_basic Nx=%d Ny=%d steps=%d threads=%d time=%.6f checksum=%.12e\n",
