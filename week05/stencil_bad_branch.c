...
static void stencil_bad_branch(double *newa, const double *olda, int Nx, int Ny)
{
...
int main(int argc, char **argv)
{
    ...
    for (int t = 0; t < steps; t++) {
        stencil_bad_branch(newa, olda, Nx, Ny);
        double *tmp = olda; olda = newa; newa = tmp;
    }
    double t1 = wall_seconds();

    printf("stencil_bad_branch Nx=%d Ny=%d steps=%d threads=%d time=%.6f checksum=%.12e\n",
